...
Postbird for DeepSeek Harness（产品名「信鸽」，包名 `dsh-plugin-office`）是一个会执行**真实外部动作**的 Agent 工具套件：发送邮件、读写本机文件。本文档说明它的威胁模型、已内置的防护机制、可配置的护栏，以及漏洞上报方式。目标读者：使用者、审查者、贡献者。
