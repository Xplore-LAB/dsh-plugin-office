        ...
        out.push({
          ...
          headers: {
            ...
            references: (Array.isArray(parsed.references) ? parsed.references : [parsed.references])
              .filter(Boolean).map(String).slice(-20)
          },
...
/**
 * Re-fetch full text for selected indexed messages over read-only IMAP.
 * Content is returned in memory and is never appended to the local index.
 */
export async function fetchMessageContents({ settings, entries, maxCharsPerMessage = 12_000 }) {
  const client = new ImapFlow({
    ...settings,
    logger: false,
    connectionTimeout: 20_000,
  ...
  try {
    ...
    for (const entry of entries) {
      const mailbox = entry.mailbox ?? "INBOX";
      if (!byMailbox.has(mailbox)) byMailbox.set(mailbox, []);
      byMailbox.get(mailbox).push(entry);
    }
    for (const [mailbox, list] of byMailbox) {
      ...
      try {
        for (const entry of list) {
          try {
            const raw = await client.fetchOne(entry.uid, { uid: true, source: true }, { uid: true });
            if (!raw?.source) {
              skipped.push({ uid: entry.uid, reason: "message not found on server" });
              continue;
            }
            const parsed = await simpleParser(raw.source);
            const fullText = (parsed.text && parsed.text.trim() !== "" ? parsed.text : stripHtml(parsed.html))
              .replace(/\s+/g, " ").trim();
            const cap = Math.max(maxCharsPerMessage, 500);
            messages.push({
              ...entry,
              snippet: fullText.slice(0, cap),
              contextSource: "imap-full",
              bodyTruncated: fullText.length > cap,
              fullTextChars: fullText.length
            });
          } catch (error) {
            skipped.push({ uid: entry.uid, reason: String(error?.message ?? error) });
          }
        }
      } finally {
        lock.release();
