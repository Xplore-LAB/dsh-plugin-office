...
// recipient/batch sizes via config, throttles per-message pacing, and logs
// every delivered message to ~/.dsh/office/mail/sent-log.jsonl. Document and
// spreadsheet tools write only into user-specified paths and refuse to
...
async function buildRows(args, cfg) {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    const displayName = args.nameColumn ? String(row[args.nameColumn] ?? "").trim() : "";
    rows.push({
      index: i + 1,
      email,
...
function applyFn(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      attachmentColumn: {
        type: "string",
        description: "Optional column whose cell holds one attachment path, or several separated by \";\"."
      },
      attachments: {
        type: "array",
        description: "Global attachment paths attached to every recipient.",
        items: { type: "string" }
      },
      maxSamples: {
        type: "integer",
