...
Version 1.5.0 exposes 22 tools. The flagship inbox workflows are `office_daily_brief`, `office_action_radar`, and `office_context_reply`. Thread summaries, action extraction, selected-tone replies, open-registration and roster-based material tracking, and cited attachment reading extend these workflows across seven campus roles. GitHub Pages serves the browser-only simulated Demo from `demo/`.
